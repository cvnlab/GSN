...
from gsn.calcmvgaussianpdf import calcmvgaussianpdf

def calcshrunkencovariance(data, 
                           leaveout = 5, 
    ...
    for p in range(len(shrinklevels)):
        ...
        if np.ndim(data) == 3:
            ...
            [pr,err] = calcmvgaussianpdf(pts = data_zeromean, 
                                             mn = mn, 
                                             c = c2,  # shrunken covariance 
                                             wantomitexp = 1) 
        else:
            
            [pr,err] = calcmvgaussianpdf(pts = data[ii], 
                                             mn = mn, 
                                             c = c2,  # shrunken covariance 
                                             wantomitexp = 1) 
